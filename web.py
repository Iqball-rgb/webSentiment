...
if uploaded_file:
    ...
    if not text_col:
        ...
        # Download hasil
        # ===== DATA UNTUK DOWNLOAD (bersih) =====
        download_cols = {text_col: "text_asli", 'Prediction_MNB': 'prediksi_multinomial'}
        if 'Prediction_GNB' in df.columns:
            download_cols['Prediction_GNB'] = 'prediksi_gaussian'

        df_download = df[list(download_cols.keys())].rename(columns=download_cols)

        # --- Download Excel (.xlsx) ---
        import io
        output_xlsx = io.BytesIO()
        with pd.ExcelWriter(output_xlsx, engine='xlsxwriter') as writer:
            df_download.to_excel(writer, index=False, sheet_name='hasil_prediksi')
        output_xlsx.seek(0)
